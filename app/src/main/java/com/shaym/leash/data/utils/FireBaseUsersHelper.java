...
import static com.shaym.leash.data.utils.CONSTANT.PUSH_TOKEN;
...
import static com.shaym.leash.data.utils.CONSTANT.USER_LOC;
import static com.shaym.leash.data.utils.CONSTANT.USER_STATUS;
public class FireBaseUsersHelper {
    ...
    public void updateUserLocation(LatLng loc){
        ...
        try{
            ...
            location.put(USER_LAT, loc.latitude);
            location.put(USER_LNG, loc.longitude);

            mDatabase.child(USERS_TABLE).child(getUid()).child(USER_LOC).setValue(location);
        }
        catch (Exception e){
        ...
        userNameRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (!dataSnapshot.exists()) {
                    ...
                    HashMap<String , Double> location = new HashMap<>();
                    location.put(USER_LAT, 32.1976639);
                    location.put(USER_LNG, 34.8811709);
                    mUser = new Profile(mCurrentUser.getDisplayName(),ROLE_USER, mCurrentUser.getEmail(), gender, profilepic, location,  mToken, mCurrentUser.getUid(), 0);
